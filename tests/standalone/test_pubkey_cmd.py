...
from application_client.zcash_command_sender import ZcashCommandSender, Errors, GetUfvkMode
...
def test_get_orchard_fvk(backend):
    ...
    client = ZcashCommandSender(backend)
    response = client.get_ufvk(path="m/32'/133'/0'", mode=GetUfvkMode.ORCHARD_FVK).data
    assert response == REF_ORCHARD_FVK_ACC_0

    client = ZcashCommandSender(backend)
    response = client.get_ufvk(path="m/32'/133'/1'", mode=GetUfvkMode.ORCHARD_FVK).data
...
def test_get_ufvk(backend):
    ...
    client = ZcashCommandSender(backend)
    response = client.get_ufvk(path="m/32'/133'/0'", mode=GetUfvkMode.UFVK).data
    ufvk = unpack_get_ufvk_response(response)
    assert ufvk == REF_UFVK_ACC_0

    client = ZcashCommandSender(backend)
    response = client.get_ufvk(path="m/32'/133'/1'", mode=GetUfvkMode.UFVK).data
    ufvk = unpack_get_ufvk_response(response)
